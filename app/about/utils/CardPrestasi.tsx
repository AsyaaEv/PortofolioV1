...
export default function CardPrestasi({ image, title, year, desc }: CardProps) {
    return (
        <>
            <div className="w-full rounded-[10px] border-2 h-[22rem] flex flex-col group overflow-hidden hover:scale-[1.01] transition-all dark:border-0 dark:bg-white/15">
                <div className="w-full h-[60%] overflow-hidden">
                    ...
                    <Image src={`/prestasi/${image} `} width={1080} height={1080} alt="" loading="lazy" className="object-cover w-full h-full rounded-tl-[10px] rounded-tr-[10px] pointer-events-none -z-10" />
                </div>
                <div className="w-full h-auto flex-col px-4 ">
                    <div className="w-full mt-2">
                        <h1 className="text-lg font-semibold">{title}</h1>
                        <h1 className="text-sm text-zinc-600 dark:text-zinc-400">{year}</h1>
                    </div>
                    <div className="w-full mt-2">
